...
type Party struct {
	...
	RobustScalingNSamples []int64
	RobustScalingInput [][]float64
	RobustScalingLCount []int64
	RobustScalingRCount []int64
...
func GenRobustParties(params bgv.Parameters, N int, NFeatures int) []*Party {
	kgen := rlwe.NewKeyGenerator(params)
	parties := make([]*Party, N)
	...
	for i := 0; i < N; i++ {
		...
		pi.RobustScalingNSamples = make([]int64, NFeatures)
		for j := 0; j < NFeatures; j++ {
			...
			pi.RobustScalingNSamples[j] = int64(length)
		}
