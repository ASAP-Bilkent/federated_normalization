...
def do_skew(skew_type, num_clients, X_data, Y_data):
    ...
    #depends on dataset
    ds_info = { 'num_clients' : num_clients , 'sample_shape' : DATASET_INPUT_SHAPE, 'num_classes':10 , 'seed':33}
...
def get_model(kernel_size = (3, 3), dropout = True, pooling = 'max', reg_alpha = 0):
    model = tf.keras.Sequential()

    model.add(tf.keras.layers.Conv2D(32, kernel_size, activation='relu', padding='same', input_shape=DATASET_INPUT_SHAPE))
    ...
    if NORMALIZATION_LAYER == 'batch_norm':
        ...
        model.add(tf.keras.layers.GroupNormalization(groups=16))
    elif NORMALIZATION_LAYER == 'layer_norm':
        model.add(tf.keras.layers.LayerNormalization())
    if pooling == 'max':
        model.add(tf.keras.layers.MaxPooling2D((2, 2)))
