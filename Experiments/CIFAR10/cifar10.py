...
def do_skew(skew_type, num_clients, X_data, Y_data):
    ...
    #depends on dataset
    ds_info = { 'num_clients' : num_clients , 'sample_shape' : DATASET_INPUT_SHAPE, 'num_classes':10 , 'seed':89}
...
def get_model(kernel_size = (3, 3), dropout = True, pooling = 'max', reg_alpha = 0.001):
    ...
    model.add(tf.keras.layers.Conv2D(64, kernel_size, activation='relu', padding='same', input_shape=DATASET_INPUT_SHAPE, kernel_regularizer=tf.keras.regularizers.l2(reg_alpha)))
    ...
    model.add(tf.keras.layers.Dense(units=1024, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(reg_alpha)))
    if dropout:
...
parameters_dict = {
    'b_skew': {
          'values': ['default','feature_0.1', 'feature_0.01', 'label_5.0', 'label_0.5', 'quantity_5.0', 'quantity_0.7']
        },
    'a_num_clients': {
          'values': [10, 20]
        },
...
sweep_id = wandb.sweep(sweep_config, project="cifar10")
...
wandb.agent(sweep_id, train) # to start a sweep
