...
def load_data(data_dir=""):
    ...
    x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size=0.2, shuffle=True, random_state=424242)
    x_train, x_vals, y_train, y_vals = train_test_split(x_train, y_train, test_size=0.1, random_state=424242)
...
def parse_arguments():
    ...
    parser.add_argument("--seed",type=int,required=False,default=424242)
...
if __name__ == "__main__":
    ...
    parameters_dict.update(
        {"dataset": {"value": "BCW"}, "experiment_run": {"value": f"{args.run}"}}
    )
